...
def layer_masking(
    model_type, bottom_model, mask_layers, device, dataset, world_size, rank
):
    if model_type == "resnet18":
        ...
        if dataset == "criteo":
            ...
            if world_size == 4:
                if 1 in mask_layers:
                    bottom_model.sequential[0] = nn.Linear(
                        in_features=196, out_features=164, bias=True
                    ).to(device)
                if 2 in mask_layers:
                    bottom_model.sequential[3] = nn.Linear(
                        in_features=164, out_features=128, bias=True
                    ).to(device)
                if 3 in mask_layers:
                    bottom_model.sequential[6] = nn.Linear(
                        in_features=128, out_features=128, bias=True
            ...
            if world_size == 6:
                if 1 in mask_layers:
                    bottom_model.sequential[0] = nn.Linear(
                        in_features=131, out_features=128, bias=True
                    ).to(device)
                if 2 in mask_layers:
                    bottom_model.sequential[3] = nn.Linear(
                        in_features=128, out_features=96, bias=True
                    ).to(device)
                if 3 in mask_layers:
                    bottom_model.sequential[6] = nn.Linear(
            ...
            if world_size == 8:
                if 1 in mask_layers:
                    bottom_model.sequential[0] = nn.Linear(
                        in_features=98, out_features=64, bias=True
                    ).to(device)
                if 2 in mask_layers:
                    bottom_model.sequential[3] = nn.Linear(
            ...
            if world_size == 10:
                if 1 in mask_layers:
                    bottom_model.sequential[0] = nn.Linear(
                        in_features=79, out_features=64, bias=True
                    ).to(device)
                if 2 in mask_layers:
                    bottom_model.sequential[3] = nn.Linear(
                        in_features=64, out_features=64, bias=True
