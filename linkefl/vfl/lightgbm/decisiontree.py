...
from linkefl.vfl.lightgbm.hist import ActiveHist
from linkefl.vfl.lightgbm.error import DisconnectedError
from linkefl.vfl.lightgbm.exception_thread import ExcThread
from linkefl.vfl.lightgbm.data_functions import (
    wrap_message,
    random_sampling,
    goss_sampling,
)
from linkefl.vfl.lightgbm.train_functions import (
    find_split,
    gh_compress_multi,
    gh_packing,
    leaf_weight,
...
class DecisionNode:
    def __init__(
        ...
        self.full_sample_tag = full_sample_tag
...
class DecisionTree:
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            max_num_leaves: maximum number of leaves on a tree
    ...
    def fit(self, gradient, hessian, bin_index, bin_split):
        ...
        sample_tag, full_sample_tag = np.zeros(sample_num, dtype=int), np.ones(sample_num, dtype=int)
        ...
        while True:
            try:
                ...
                # start building tree
                self.root = self._build_tree(sample_tag, full_sample_tag)
    ...
    def _build_tree(self, sample_tag, full_sample_tag):
        # 1. initial root node.
        num_leaves = 0     # used to limit tree
        root = self._gen_node(sample_tag, full_sample_tag, depth=0)

        # 2. generate leaf nodes
        split_node_candidates = PriorityQueue()
        ...
        while not split_node_candidates.empty() and num_leaves + split_node_candidates.qsize() < self.max_num_leaves:
            ...
            if node.depth < self.max_depth \
               and node.sample_tag.sum() >= self.min_split_samples \
               and node.split_gain > self.min_split_gain:

                # split node
                feature_id_origin, record_id, sample_tag_left, full_sample_tag_left = self._get_split_info(node)
                sample_tag_right = node.sample_tag - sample_tag_left
                full_sample_tag_right = node.full_sample_tag - full_sample_tag_left

                # generate and store child nodes
                left_node, right_node = self._gen_child_node(node, sample_tag_left, sample_tag_right,
                                                             full_sample_tag_left, full_sample_tag_right)
                ...
                self.feature_importance_info['gain'][f'client{node.party_id}_feature{feature_id_origin}'] += node.split_gain
                ...
                # as leaf node, update pred
                if self.task == "multi":
                    update_temp = np.dot(node.full_sample_tag.reshape(-1, 1), node.value.reshape(1, -1))
                else:
                    update_temp = np.dot(node.full_sample_tag, node.value)
        ...
        while not split_node_candidates.empty():
            ...
            if self.task == "multi":
                ...
                update_temp = np.dot(node.full_sample_tag, node.value)
            self.update_pred += np.dot(node.full_sample_tag, node.value)
            num_leaves += 1
    ...
    def _gen_node(self, sample_tag, full_sample_tag, depth, hist_list=None):
        # 1. compute leaf weight
        if self.task == "multi":
        ...
        # 3. default as a leaf node
        node = DecisionNode(party_id=0, value=leaf_value,
                            hist_list=hist_list, sample_tag=sample_tag, full_sample_tag=full_sample_tag,
                            split_party_id=party_id, split_feature_id=feature_id,
                            split_bin_id=split_id, split_gain=max_gain,
                            depth=depth)

        return node

    def _gen_child_node(self, node, sample_tag_left, sample_tag_right, full_sample_tag_left, full_sample_tag_right):
        if sample_tag_left.sum() < sample_tag_right.sum():
            # 生成左孩子节点
            left_node = self._gen_node(sample_tag_left, full_sample_tag_left, node.depth + 1)

            hist_list_right = []
            for current_hist, left_hist in zip(node.hist_list, left_node.hist_list):
                if current_hist is None or left_hist is None:
                    hist_list_right.append(None)
                else:
                    hist_list_right.append(current_hist - left_hist)
            right_node = self._gen_node(sample_tag_right, full_sample_tag_right, node.depth + 1, hist_list_right)

        else:
            right_node = self._gen_node(sample_tag_right, full_sample_tag_right, node.depth+1)

            hist_list_left = []
            for current_hist, right_hist in zip(node.hist_list, right_node.hist_list):
                if current_hist is None or right_hist is None:
                    hist_list_left.append(None)
                else:
                    hist_list_left.append(current_hist - right_hist)
            left_node = self._gen_node(sample_tag_left, full_sample_tag_left, node.depth + 1, hist_list_left)
    ...
    def _get_split_info(self, node):
        if node.split_party_id == 0:
            ...
            feature_id_origin, record_id, sample_tag_left, full_sample_tag_left = self._save_record(
                node.split_feature_id, node.split_bin_id, node.sample_tag, node.full_sample_tag
            )
            self.logger.log(f"threshold saved in record_id: {record_id}")
        else:
            # split in passive party
            self.messengers[node.split_party_id - 1].send(
                wrap_message(
                    "record", content=(node.split_feature_id, node.split_bin_id, node.sample_tag, node.full_sample_tag)
                )
            )
            ...
            feature_id_origin, record_id, sample_tag_left, full_sample_tag_left = data["content"]

        return feature_id_origin, record_id, sample_tag_left, full_sample_tag_left

    def _save_record(self, feature_id, split_id, sample_tag, full_sample_tag):
        # Map the selected feature index to the original feature index
        feature_id_origin = self.feature_index_selected[feature_id]

        record = np.array([feature_id_origin, self.bin_split[feature_id_origin][split_id]]).reshape(1, 2)
        ...
        sample_tag_left = np.array(sample_tag.copy())  # avoid modification on sample_tag_selected
        sample_tag_left[self.bin_index_selected[:, feature_id].flatten() > split_id] = 0

        full_sample_tag_left = np.array(full_sample_tag.copy())
        full_sample_tag_left[self.bin_index_selected[:, feature_id].flatten() > split_id] = 0
        # print(full_sample_tag.sum(), full_sample_tag_left.sum())
        return feature_id_origin, record_id, sample_tag_left, full_sample_tag_left

    def _predict_value(self, x_sample, sample_id,
                       tree_node: DecisionNode = None):
        """predict a sample"""
    ...
    def _get_hist_list(self, sample_tag):
        ...
        return hist_list, party_id, feature_id, split_id, max_gain
