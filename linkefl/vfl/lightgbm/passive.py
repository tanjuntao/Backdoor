...
from linkefl.vfl.lightgbm.data_functions import get_bin_info, wrap_message
from linkefl.vfl.lightgbm.hist import PassiveHist
...
class PassiveTreeParty(BaseModelComponent):
    ...
    def train(self, trainset, testset):
        ...
        while True:
            ...
            if data["name"] == "gh":
                ...
                feature_id, split_id, sample_tag, full_sample_tag = data["content"]

                feature_id_origin, record_id, sample_tag_left, full_sample_tag_left = self._save_record(
                    feature_id, split_id, sample_tag, full_sample_tag
                )
                self.logger.log(f"threshold saved in record_id: {record_id}")
                self.messenger.send(
                    wrap_message("record",
                                 content=(feature_id_origin, record_id, sample_tag_left, full_sample_tag_left))
                )
    ...
    def _save_record(self, feature_id, split_id, sample_tag, full_sample_tag):
        ...
        self.feature_importance_info_tree['cover'][f'feature{feature_id_origin}'] += sum(sample_tag)
        ...
        sample_tag_left[self.bin_index[:, feature_id_origin].flatten() > split_id] = 0
        full_sample_tag_left = full_sample_tag
        full_sample_tag_left[self.bin_index[:, feature_id_origin].flatten() > split_id] = 0
