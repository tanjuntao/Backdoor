...
from linkefl.vfl.tree.core.error import DisconnectedError, ExcThread
from linkefl.vfl.tree.core.hist import ActiveHist
from linkefl.vfl.tree.core.tree_node import _DecisionNode
from linkefl.vfl.tree.utils.gh import GH
from linkefl.vfl.tree.utils.leaf_value import LeafValue
from linkefl.vfl.tree.utils.sample import Sample
from linkefl.vfl.tree.utils.split import find_split
from linkefl.vfl.tree.utils.util_func import wrap_message
...
class DecisionTree:
    def __init__(
        ...
        *,
        ...
        self.model_phase = "train"  # train, inference
    ...
    def fit(self, gradient, hessian, bin_index, bin_split):
        ...
        if self.sampling_method == "uniform":
            selected_g, selected_h, selected_idx = Sample.random_sampling(
                gradient, hessian, self.subsample
            )
        elif self.sampling_method == "goss":
            selected_g, selected_h, selected_idx = Sample.goss_sampling(
    ...
    def _get_gh_send(self, sample_num, selected_g, selected_h, selected_idx):
        if self.task == "regression" or self.task == "binary":
            selected_gh_int, self.h_length, self.gh_length = GH.gh_packing(
            ...
            if self.crypto_type == Const.PLAIN:
                ...
                selected_gh_enc = self.crypto_system.encrypt_data(
                    plain_data=selected_gh_int, pool=self.pool
                )
                for i, idx in enumerate(selected_idx):
                    gh_send[idx] = selected_gh_enc[i]
                gh_send = np.array(gh_send, dtype=object)
            else:
                raise ValueError("Unsupported crypto_type.")

        elif self.task == "multi":
            selected_gh_compress, self.h_length, self.gh_length = GH.gh_compress_multi(
                selected_g,
                selected_h,
            ...
            if self.crypto_type == Const.PLAIN:
                for i, sample_idx in enumerate(selected_idx):
                    for j in range(sample2enc_num):
                        gh_send[sample_idx][j] = selected_gh_compress[i][j]
    ...
    def _build_tree_xgb(self, sample_tag_selected, sample_tag_unselected):
        ...
        while len(split_node_candidates) > 0:
            ...
            if (
                ...
                # compute leaf weight
                if self.task == "regression" or self.task == "binary":
                    leaf_value = LeafValue.leaf_weight(
                    ...
                    leaf_value = LeafValue.leaf_weight_multi(
                        self.gh, node.sample_tag_selected, self.reg_lambda
                    )
                    update_temp = np.dot(
    ...
    def _predict_value(self, x_sample, sample_id, tree_node: _DecisionNode):
        ...
        if tree_node.party_id == 0:
            ...
            branch_tag = True if x_sample[feature_id] > threshold else False
        else:
            ...
            if self.drop_protection:
                ...
                if not passive_party_connected:
                    raise DisconnectedError(
                        disconnect_phase="predict phase",
    ...
    def _gen_node(
        ...
        if hist_list is None:
            ...
            party_id, feature_id, split_id, max_gain = find_split(
                hist_list, self.task, self.reg_lambda, self.reg_gamma
            )
        ...
        try:
            ...
            for i, messenger in enumerate(self.messengers, 1):
                ...
                _, feature_id, split_id, max_gain = find_split(
                    [active_hist], self.task, self.reg_lambda, self.reg_gamma
                )
    ...
    def _process_passive_hist(self, messenger, i, q: queue.Queue):
        ...
        _, feature_id, split_id, max_gain = find_split(
            [passive_hist], self.task, self.reg_lambda, self.reg_gamma
        )
