...
from linkefl.feature.transform import AddIntercept, add_intercept
from linkefl.modelio import NumpyModelIO

# from linkefl.vfl.linear import BaseLinearActive
from linkefl.vfl.linear.base_multi import (
    BaseLinearActive_disconnection,
    BaseLinearPassive,
)
...
class ActiveLinReg_disconnection(BaseLinearActive_disconnection):
    ...
    def train(self, trainset, testset):
        ...
        for epoch in range(self.epochs):
            if epoch % self.val_freq == 0:
                if self.reconnection:
                    for id in range(self.world_size):
                        if not passive_party[id]:
                            passive_party[id] = self.try_to_connection(
                                id=id, reconnection_port=self.reconnection_port
                            )
                            passive_party[id] = self.messenger.send(
                                epoch, id, passive_party[id]
                            )
                ...
                for id in range(self.world_size):
                    if passive_party[id]:
                        print(
                            colored(
                                "passive_party {} status: connection", "red"
                            ).format(id)
                        )
                    else:
                        print(
                            colored(
                                "passive_party {} status: disconnection", "red"
                            ).format(id)
                        )
            ...
            for batch in range(n_batches):
                ...
                #     full_wx += passive_wx
                for id in range(self.world_size):
                    passive_wx, passive_party[id] = self.messenger.recv(
                        id, passive_party[id]
                    )
                    if passive_party[id]:
                ...
                for id in range(self.world_size):
                    passive_party[id] = self.messenger.send(
                        enc_residue, id, passive_party[id]
                    )

                for id in range(self.world_size):
                    enc_passive_grad, passive_party[id] = self.messenger.recv(
                        id, passive_party[id]
                    )
                    _begin = time.time()
                    if passive_party[id]:
                        ...
                        passive_party[id] = self.messenger.send(
                            passive_grad, id, passive_party[id]
                        )
                    compu_time += time.time() - _begin
            ...
            if epoch % self.val_freq == 0:
                ...
                result, output_temp_test, passive_party = self.validate(
                    testset, output_temp_test, passive_party
                )
    ...
    def validate(self, valset, output_temp_test, passive_party):
        ...
        return {"loss": loss, "r2": r2}, output_temp_test, passive_party
    ...
    def train(self, trainset, testset):
        ...
        if self.crypto_type in (Const.PAILLIER, Const.FAST_PAILLIER):
            ...
            x_encode = BaseLinearPassive._encode(
                getattr(self, "x_train"), public_key, self.precision
            )
            print("Done!")
