...
class ActiveLogReg_disconnection(BaseLinearActive_disconnection):
    ...
    def train(self, trainset, testset):
        ...
        for epoch in range(self.epochs):
            ...
            for id in range(self.world_size):
                if passive_party[id]:
                    print(
                        colored("passive_party {} status: connection", "red").format(id)
                    )
                else:
                    print(
                        colored("passive_party {} status: disconnection", "red").format(
                            id
                        )
                    )
    ...
    def validate(self, valset, output_temp_test, passive_party):
        ...
        return {"acc": accuracy, "f1": f1, "auc": auc}, passive_party, output_temp_test
