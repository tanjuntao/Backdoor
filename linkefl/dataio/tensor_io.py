...
class SklearnDataset(Dataset):
    def __init__(self,
        ...
        self.train = train
        self.role = role
        assert self.role in (Const.ACTIVE_NAME, Const.PASSIVE_NAME), 'Invalide role'
        self.attacker_features_frac = attacker_features_frac
        self.permutation = permutation
        self.transform = transform
        self.target_transform = target_transform

        if self.dataset_name == 'cancer':
...
class TorchDataset(Dataset):
    def __init__(self,
        ...
        if dataset_name == 'fashion_mnist':
            self.torch_dataset = datasets.FashionMNIST(root='../data',
                                                       train=train,
                                                       download=True,
                                                       transform=transform,
                                                       target_transform=target_transform)
        elif dataset_name == 'mnist':
            self.torch_dataset = datasets.MNIST(root='../data',
                                                train=train,
                                                download=True,
                                                transform=transform,
                                                target_transform=target_transform)
            ...
            self.torch_dataset = datasets.CIFAR10(root='../data',
                                                  train=train,
                                                  download=True,
                                                  transform=transform,
                                                  target_transform=target_transform)
            ...
            self.torch_dataset = datasets.SVHN(root='../data',
                                               split=split,
                                               download=True,
                                               transform=transform,
                                               target_transform=target_transform)
...
class CSVDataset(Dataset):
    def __init__(self,
        ...
        self.role = role
        assert role in (Const.ACTIVE_NAME, Const.PASSIVE_NAME), 'Invalide role'
        self.train = train
        self.attacker_features_frac = attacker_features_frac
        self.permutation = permutation
        self.transform = transform
...
def get_tensor_dataset(dataset_name,
                       role,
                       train,
                       attacker_features_frac=0.5,
                       permutation=None,
