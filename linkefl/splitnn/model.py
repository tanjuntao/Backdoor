...
"""
The following classes are designed to split widely used models for two-party
and u-shape split learning. We can split models into different parts and allocate them
to the server and the client.
The parameters _in_layer and _out_layer are the splitting strategy.

e.g.
If a model has 5 layers, we define that it has 6 split layers
including (0, 1, 2, 3, 4, 5). if we want to extract layer2 and layer3 of the model,
the parameter _in_layer = 1 and _out_layer = 3; If we want to extract layer1 to layer4
of the model, the parameter _in_layer = 0 and _out_layer = 4.
"""
...
class SplitLenet5(nn.Module):
    def __init__(self, _in_layer=0, _out_layer=5):
        ...
        if self.out_layer <= self.in_layer:
            raise ValueError(
                "Invalid in layer and out layer: out layer should be larger than in"
                " layer."
            )
    ...
    def forward(self, x):
        ...
        if self.out_layer == 1:
            return x
        if self.in_layer in [0, 1]:
            x = self.layer2(x)
        if self.out_layer == 2:
            return x
        if self.in_layer in [0, 1, 2]:
            x = self.layer3(x)
        if self.out_layer == 3:
            return x
        if self.in_layer in [0, 1, 2, 3]:
            x = self.layer4(x)
        if self.out_layer == 4:
            return x
        x = self.layer5(x)
