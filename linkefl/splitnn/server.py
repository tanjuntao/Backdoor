...
if __name__ == "__main__":
    ...
    if model_name == "lenet5":
        if not u_shape_flag:
            ...
            server_model = SplitLenet5(
                _in_layer=_split_layer[0], _out_layer=_split_layer[1]
            )
    elif model_name[0:3] == "VGG":
        if not u_shape_flag:
            server_model = SplitVGG(model_name, _in_layer=_split_layer[0])
        else:
            server_model = SplitVGG(
                model_name, _in_layer=_split_layer[0], _out_layer=_split_layer[1]
            )
    else:
