...
def scale(dataset):
    if isinstance(dataset, NumpyDataset):
        ...
        if dataset.has_label:
            dataset.get_dataset()[:, 2:] = scaled_feats
        else:
            dataset.get_dataset()[:, 1:] = scaled_feats
    elif isinstance(dataset, TorchDataset):
        scaled_feats = preprocessing.scale(dataset.features.numpy())
        if dataset.has_label:
            dataset.get_dataset()[:, 2:] = torch.from_numpy(scaled_feats)
        else:
            dataset.get_dataset()[:, 1:] = torch.from_numpy(scaled_feats)
    else:
...
def normalize(dataset, norm=Const.L2):
    if isinstance(dataset, NumpyDataset):
        ...
        if dataset.has_label:
            dataset.get_dataset()[:, 2:] = normalized_feats
        else:
            dataset.get_dataset()[:, 1:] = normalized_feats
    elif isinstance(dataset, TorchDataset):
        normalized_feats = preprocessing.normalize(dataset.features.numpy(), norm=norm)
        if dataset.has_label:
            dataset.get_dataset()[:, 2:] = torch.from_numpy(normalized_feats)
        else:
            dataset.get_dataset()[:, 1:] = torch.from_numpy(normalized_feats)
    else:
        raise TypeError('dataset should be an instance of'
                        'NumpyDataset or TorchDataset')
...
def add_intercept(dataset):
    if isinstance(dataset, NumpyDataset):
        if dataset.has_label:
            ...
            new_np_dataset = np.c_[dataset.get_dataset(), np.zeros(n_samples)]
        ...
        if dataset.has_label:
            n_samples = dataset.n_samples
            new_torch_dataset = torch.cat((dataset.get_dataset(), torch.zeros(n_samples)),
                                          dim=1)
