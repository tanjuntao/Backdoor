...
class LogisticRegressionCrossEntropy:
    ...
    def get_params(self):
        ...
        return {"coef": _coef, "intercept": _intercept}
    ...
    def loss(self, x_train, y_train, batch_idxes):
        ...
        if self.penalty == "none":
            ...
            reg_loss = 1.0 / 2 * self.lamda * (self.w**2).sum()
        else:
            raise ValueError("Regularization method not supported at this moment")
    ...
    def grad(self, x_train, y_train, batch_idxes):
        ...
        train_grad = (-1.0 / len(batch_idxes)) * (
            r[:, np.newaxis] * x_train[batch_idxes]
        ).sum(axis=0)
    ...
    def fit(self, x_train, y_train):
        ...
        # load best model
        if self.warm_start and os.path.exists("./model/{}.pkl".format(self.model_name)):
        ...
        for epoch in range(self.max_iter):
            ...
            if hasattr(self, "x_test"):
                ...
                if self.validate:
                    _test_data = np.c_[self.x_test, np.ones(self.x_test.shape[0])]
                    test_loss = self.loss(
                        _test_data,
                        self.y_test,
                        batch_idxes=np.arange(self.x_test.shape[0]),
                    )
                    print("Test loss: {}".format(test_loss))
                    print(
                        "Test Acc: {}, Test Auc: {}".format(
                            scores["Accuracy"], scores["auc_score"]
                        )
                    )
                ...
                if (
                    (cur_acc > best_acc and cur_auc > best_auc_score)
                    or (cur_acc - best_acc >= 0.01)
                    or (cur_auc - best_auc_score >= 0.01)
                ):
    ...
    def predict(self, X, threshold=0.5):
        ...
        predictions = [1 if prob > threshold else self.negative_label for prob in probs]
...
def get_dataset(name):
    ...
    if name == "cancer":
        ...
        train_path = os.path.join(
            curr_path, "../LinkeFL/data/tabular/census_income_train.csv"
        )
        test_path = os.path.join(
            curr_path, "../LinkeFL/data/tabular/census_income_test.csv"
        )
        ...
        train_path = os.path.join(
            curr_path, "../LinkeFL/data/tabular/give_me_some_credit_train.csv"
        )
        test_path = os.path.join(
            curr_path, "../LinkeFL/data/tabular/give_me_some_credit_test.csv"
        )
        train_set = np.genfromtxt(train_path, delimiter=",")
        test_set = np.genfromtxt(test_path, delimiter=",")
        x_train, y_train = train_set[:, 2:], train_set[:, 1]
        ...
        train_path = os.path.join(
            curr_path, "../LinkeFL/data/tabular/epsilon_train.csv"
        )
        test_path = os.path.join(curr_path, "../LinkeFL/data/tabular/epsilon_test.csv")
        train_set = np.genfromtxt(train_path, delimiter=",")
        test_set = np.genfromtxt(test_path, delimiter=",")
        x_train, y_train = train_set[:, 2:], train_set[:, 1]
        x_test, y_test = test_set[:, 2:], test_set[:, 1]
...
if __name__ == "__main__":
    clf = LogisticRegressionCrossEntropy(
        penalty="l2",
        lamda=0.01,
        lr=0.01,
        max_iter=200,
